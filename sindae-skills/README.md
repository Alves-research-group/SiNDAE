...
- `CLAUDE.md`, the operating instructions: the role, the mandatory workflow, the
  human-in-the-loop equation gate, and the honesty rules.
...
Pick the option that matches how you use Claude. Option A is the intended one and
needs no edits to any file.

### Option A: Claude Code, in your own modeling project (recommended)
...
your data and your model code live:

```bash
mkdir my-bioreactor-model
cp /path/to/SiNDAE/sindae-skills/CLAUDE.md /my-bioreactor-model/.claude/
cp /path/to/SiNDAE/sindae-skills/skills    /my-bioreactor-model/.claude/
...
```markdown
For any hybrid modeling work, follow @sindae-skills/CLAUDE.md.
```

Verify it took effect by opening Claude in the directory of you project and running the command in the claude interface:
```python
/memory
```
...
3. Upload the four files in `skills/` as project knowledge.

## What a session looks like

The workflow in `CLAUDE.md` is mandatory and ordered:
...
2. **Draft.** The governing equations are written out, with the network outputs
   marked. No code yet.
3. **Confirm.** Claude renders the full model as LaTeX and stops. You approve or
...
| File | Load when |
|------|-----------|
| `skills/hybridization-strategy.md` | The user is still deciding *what* to learn: degree of hybridization, the accuracy vs data-cost trade-off, extrapolation, interpretability. |
